...
impl Plugin for RenderPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, spawn_visual_stage)
           .add_systems(Update, (
    ...
    for (mut tf, mut anim, state) in &mut query {
        ...
        match state {
            ...
            PetVisualState::Sleeping => {
                ...
                tf.rotate_y(time.delta_secs() * 0.04);
            }
        }
    }
}
...
fn update_pet_expressions(
    ...
    mut mouths: Query<(&ChildOf, &mut Transform), (With<PetMouth>, Without<PetEye>)>,
) {
    // 1. Update Head Material Color when FACES state changes
    for (faces_state, mat_handle) in changed_heads.iter() {
    ...
    // 2. Adjust eye sizes or shapes based on Focus
    ...
    // 3. Adjust mouth based on Action
